...
class Quantity(object):
    ...
    def __str__(self):
        ...
        return format(mag, format_spec) + ' ' + symbol
    ...
    def __format__(self, format_spec):
        ...
        mag, symbol, _ = self._getRepresentTuple()
        #import pdb; pdb.set_trace()
        if format_spec == '':
            format_spec = _
        return format(mag, format_spec) + ' ' + symbol
...
createUnit('hr hour', 3600 * s)
createUnit('j jiffy', .016 * s)
createUnit('ja jiffy_alternate', 10 * ms)
createUnit('ke_quarter_of_an_hour ', 15 * min)
createUnit('ke_traditional ', 14.4 * min)
...
createUnit('cubic_foot  cu_ft ', 0.028316846592 * m3)
createUnit('cubic_inch  cu_in ', 16.387064e-6 * m3)
createUnit('cubic_mile  cu_mi ', 4168181825.440579584 * m3)
createUnit('cubic_yard  cu_yd ', 0.764554857984 * m3)
...
#Length
createUnit('nm nanometre', 1e-9 * m)
...
createUnit('ft foot_International', 0.3048 * m)
...
createUnit('ton_long  long_tn_or_ton ', 1016.0469088 * kg)
createUnit('ton_short  sh_tn ', 907.18474 * kg)
createUnit('t tonne_mts_unit', 1000 * kg)
